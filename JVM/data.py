...
parser.add_argument("--path_to_file", "-p", type=str, default="data/corpus.txt", help="Path to data.txt file")
...
def build_training_corpus(vocab_size):
    ...
    with open('{}/corpus.txt'.format(dir), 'w', encoding='utf-8') as corpus:
        with open(args.path_to_file, 'r', encoding='utf-8') as f:
            lines = f.readlines()
            for i, line in enumerate(lines):
                if i % 1000 == 0:
...
if __name__ == "__main__":
    if not os.path.exists('data/lexicon.pkl'):
        build_lexicon(args.path_to_file)
